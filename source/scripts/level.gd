...
func _ready():
	...
	# Network signal connection must happen for EVERYONE (Server, Host, Client)
	# to handle player spawning.
	...
	# Only Server needs to bake navmesh (if runtime) or do server-specific setup
...
func get_spawn_point() -> Vector3:
	...
	return Vector3(spawn_point.x, 2.0, spawn_point.y)
...
func _input(event):
	...
	var action_pressed = event.is_action_pressed("open_chat_or_send_message")
	
	if action_pressed or is_enter:
		print("Debug: Chat Input. Action: ", action_pressed, " Key: ", is_enter, " Visible: ", multiplayer_chat.visible)
		
		if multiplayer_chat.visible and multiplayer_chat.message.has_focus():
			# If chat is visible and focused, send message
			multiplayer_chat._on_send_pressed()
			get_viewport().set_input_as_handled()
		elif not multiplayer_chat.visible:
			# If chat is not visible, open and focus it
			multiplayer_chat.show()
			multiplayer_chat.message.grab_focus()
			get_viewport().set_input_as_handled()
			
	elif event.is_action_pressed("close_chat"):
		if multiplayer_chat.is_chat_visible():
			multiplayer_chat.hide()
			get_viewport().set_input_as_handled()
		
	# Old inventory/debug input remains
...
func is_chat_focused() -> bool:
	return multiplayer_chat.is_typing()
...
func spawn_projectile(origin: Vector3, direction: Vector3, shooter_id: int):
	if not multiplayer.is_server():
		return
		
	var proj_scene = load("res://source/scenes/gameplay/projectile.tscn")
