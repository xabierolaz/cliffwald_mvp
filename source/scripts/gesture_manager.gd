...
func _build_templates() -> void:
	...
	# 1. KINETIC PULSE (Ignis/Universal) - Line
	var line_pts = [Vector2(0, 0), Vector2(0, -100)] # Swipe UP
	templates[&"lineo"] = _normalize_pipeline(line_pts)

	# 2. AEGIS (Vesper/Universal) - Circle
	# Approximation of a circle with 32 points
	templates[&"circolo"] = _normalize_pipeline(_regular_polygon_points(32, 100.0, -PI/2))

	# 3. PYROCLASM (Ignis) - Triangle
	# Triangle pointing UP (or down? let's try UP first)
	var raw_tri = _regular_polygon_points(3, 100.0, -PI/2) # Point up
	# Add permutations (start from any corner, CW or CCW)
	_add_permutations_for_shape(&"triangolo", raw_tri, 3)

	# 4. STASIS (Axiom) - Square
	var raw_sq = _regular_polygon_points(4, 100.0, -PI/4) # Square aligned to axes
	_add_permutations_for_shape(&"squaro", raw_sq, 4)
...
func _resample(pts: PackedVector2Array, n: int) -> PackedVector2Array:
		...
		var D = 0.0
		var _new_pts = PackedVector2Array()
		_new_pts.append(pts[0])
		var i = 1
		while i < pts.size():
			...
			if (D + d) >= I:
				...
				D += d
				i += 1
		while _new_pts.size() < n:
			_new_pts.append(pts[-1])
		return _new_pts
